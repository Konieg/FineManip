...
public:
    Communication();  // 构造函数
    void getPosition(geometry_msgs::msg::Pose& target) const;  // 修改为引用传递
...
private:
    void poseCallback(const geometry_msgs::msg::Pose::SharedPtr msg);  // 订阅回调函数
    bool is_valid_pose(const geometry_msgs::msg::Pose& msg);  // 检查Pose是否有效的辅助函数
    bool is_processing_message = true;
    rclcpp::Subscription<geometry_msgs::msg::Pose>::SharedPtr pose_subscriber;  // 订阅者
    ...
    geometry_msgs::msg::Pose target_pose_;  // 存储接收到的目标位姿
};
